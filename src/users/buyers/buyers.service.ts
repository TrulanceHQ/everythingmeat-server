...
export class BuyersService {
  ...
  async UpdateCartQty(cartId: string, qty: number) {
    try {
      ...
      const cart = await this.cartModel.findOne({ _id: cartId });
      if (!cart) throw new BadRequestException('Cart does not exist');
      const resp = await this.cartModel.updateOne(
        { _id: cartId },
        { slot: qty },
      );
      return resp;
  ...
  async createOrder(buyerId: any, res: Response): Promise<any> {
    try {
      ...
      const usersCart = await this.cartModel
        .find({ status: true, buyer: buyer._id })
        .populate('prod');
  ...
  async saveOrder(id: any) {
    try {
      ...
      for (let index = 0; index < usersCart.length; index++) {
        ...
        await this.sellerService.createSale(usersCart[index], savedOrder);
        ...
        await userCart.updateOne({ _id: userCart._id }, { status: false });
      }
    } catch (error) {
      console.log(error);
    }
  ...
  async paymentCallBack(flwDto: FlWRedirectDto) {
    if (flwDto.status === 'successful') {
      const transactionDetails = await this.flwModel.findOne({
        ref: flwDto.tx_ref,
      });
      const response = await flw.Transaction.verify({
        id: flwDto.transaction_id,
      });
      if (
