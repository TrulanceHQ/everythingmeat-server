  ...
  async sendEmail(to: string, subject: string, template: string, context: any) {
    ...
    if (template === 'verification-code') {
      htmlTemplate = `
        ...
        <p><strong>Verification Code:</strong> ${context.code}</p>
        <p>This code is valid for only 2 minutes.</p>
        <br>
        <p>Best Regards,</p>
        <p>EverythingMeat Team</p>
      `;
    } else if (template === 'reset-password') {
      htmlTemplate = `
        <h3>Password Reset Request</h3>
        <p>Click the link below to reset your password. This link is valid for only 10 minutes:</p>
        ...
        <p>EverythingMeat Team</p>
      `;
    } else if (template === 'temporary-password') {
      htmlTemplate = `
        <h3>Welcome!</h3>
    ...
    try {
      const info = await this.transporter.sendMail({
        from: `"EverythingMeat" <${process.env.BREVO_FROM}>`,
        to,
        subject,
        html: htmlTemplate,
      });
